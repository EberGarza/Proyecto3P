...
void Opciones::Mostrar() {
    ...
    while (ventana.isOpen() && enOpciones) {
        ...
        if (!textos.empty()) {
            std::string resolStr = std::string(u8"Resolucion: < ") + std::to_string(resoluciones[resolucionActual].x) + "x" + std::to_string(resoluciones[resolucionActual].y) + " >";
